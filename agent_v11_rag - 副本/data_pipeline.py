...
数据检查逻辑：
1. 优先检查 base（最终数据集），如果存在则直接使用
2. 如果 base 不存在，检查 processed_data，如果存在则只需划分
3. 如果都不存在，检查 raw_data，从原始数据开始处理
"""
...
class DataPipeline:
    ...
    def prepare_dataset(self, raw_dir: str, processed_dir: str,
        ...
        检查逻辑（优先级从高到低）：
        1. 检查 base（最终数据集）- 如果存在且完整，直接使用
        2. 检查 processed_data - 如果存在，只需划分（不强制要求）
        3. 检查 raw_data - 如果需要从头处理
        """
        print("\n" + "=" * 60)
