...
class ThresholdSearcher:
    ...
    def _compute_score(self, metrics: Dict[str, float]) -> float:
        ...
        # 计算差距
        closed_gap = max(0, obj["min_closed_acc"] - metrics["closed_acc"])
        open_gap = max(0, obj["target_open_auc"] - metrics["open_auc"])
        reject_excess = max(0, metrics["reject_rate"] - obj["max_reject_rate"])
        
        # 如果满足所有约束，用指标值计算得分
        if closed_gap == 0 and open_gap == 0 and reject_excess == 0:
            return (
                metrics["closed_acc"] * 2.0 +
                metrics["open_auc"] * 1.5 +
                (1 - metrics["reject_rate"]) * 0.5
            )
        
        # 否则用惩罚项
        penalty = closed_gap * 5.0 + open_gap * 3.0 + reject_excess * 1.0
        base = metrics["closed_acc"] + metrics["open_auc"]
    ...
    def _select_best(self, pareto_front: List[SearchResult]) -> SearchResult:
        ...
        for r in pareto_front:
            m = r.metrics
            if (m["closed_acc"] >= obj["min_closed_acc"] and
                m["open_auc"] >= obj["target_open_auc"] and
                m["reject_rate"] <= obj["max_reject_rate"]):
                feasible.append(r)
