...
"""
优化后的模型训练和评估模块
- 所有参数从 config.yaml 读取，不再写死
- 支持从 main 模块调用 train(cfg) 和 evaluate(cfg)
"""
import os
import re
import json
...
def _build_label_map_from_triple(mid_root: Path, save_path: str, num_known_devices: int = None) -> Dict[str, int]:
    ...
    Args:
        mid_root: 训练数据目录（仅用于回退）
        save_path: 标签映射保存路径
        num_known_devices: 已知设备数量（从 config 读取，确保一致性）
    
    Returns:
        标签映射字典 {DeviceName: label_index}
    """
...
class ThreeBandDataset(Dataset):
    ...
    def __init__(self, root_low: Path, root_mid: Path, root_high: Path,
        ...
        """
        Args:
            root_low, root_mid, root_high: 三个频带的数据目录
            label_map: 标签映射字典
            train: 是否为训练模式（影响数据增强）
            cfg: 配置字典
            filter_unknown: 是否过滤掉未知标签（label=-1）的样本
                           - True: 用于 train/val，只保留已知类别
                           - False: 用于 test，保留所有样本（包括未知类别）
        """
...
# 预训练权重路径映射
# ============================================================================
PRETRAINED_WEIGHTS_DIR = "/home/wangyb/pretrained_models"
...
class ConvNeXtCosFace(nn.Module):
    def __init__(self, n_classes, backbone="convnext_tiny", embed_dim=256, 
        ...
        Args:
            ...
            embed_dim: 嵌入维度（未使用，保留兼容性）
            pretrained_try: 是否尝试加载预训练权重
            channels_last: 是否使用 channels_last 内存格式
            pretrained_dir: 预训练权重目录（可选，覆盖默认路径）
        """
        super().__init__()
