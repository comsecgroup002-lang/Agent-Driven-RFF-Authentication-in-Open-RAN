...
class DataPipeline:
    ...
    def _parse_split_config(self, cfg: dict) -> DatasetSplitConfig:
        ...
        return DatasetSplitConfig(
            ...
            iq_file_end=ds_cfg.get("iq_file_end", 5),
        )
...
def update_aug_config_from_advice(cfg: dict, param_changes: Dict) -> dict:
    """Backward-compatible helper for the optional reprocessing branch.

    The manuscript-aligned lightweight guidance path does not call this
    function; it operates only in threshold space without retraining.
    """
    import copy
    cfg = copy.deepcopy(cfg)

    if "signal_augment" not in cfg:
        cfg["signal_augment"] = {}

    sa = cfg["signal_augment"]

    # 参数范围映射
    param_mapping = {
        "retain_ratio": (0.3, 0.7),
        "channel_aug_prob": (0.7, 1.0),
        "engineering_aug_prob": (0.4, 0.9),
    ...
    for key, value in param_changes.items():
        if key in param_mapping:
            min_val, max_val = param_mapping[key]
...
if __name__ == "__main__":
    test_cfg = {
        "paths": {"base": "/test", "label_map": "labels.json"},
        "data": {},
        "signal_processing": {},
        "signal_augment": {"retain_ratio": 0.5},
        "dataset_split": {"num_known_devices": 10, "total_devices": 11},
    }

    pipeline = DataPipeline(test_cfg)
    print(f"retain_ratio: {pipeline.aug_config.retain_ratio}")
    print("✅ Test passed!")
